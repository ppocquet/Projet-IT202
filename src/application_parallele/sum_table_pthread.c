...
sum(struct tableau *T) {
	...
	err = pthread_create(&thread1,NULL, (void *(*)(void *)) sum, (void *)&T1);
	assert(!err);
	err = pthread_create(&thread2,NULL, (void *(*)(void *)) sum, (void *)&T2);
	assert(!err);
	...
	pthread_exit((int)retval1+(int)retval2);
    }
    else {
	int temp = T->ptr[T->i];
...
main(int argc, char **argv) {
    ...
    err = pthread_create(&thread, NULL, (void * (*) (void *))sum,(void*) &tab);
    assert(!err);
